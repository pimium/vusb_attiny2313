...
static void usage(char *name) {
  fprintf(
      ...
      "  -w      write data to a address (-w \"address,data\")\n"
      "  -r      read data from an address (-w \"address,data\").\n"
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "m:a:c:w:r:l:u:d:h")) != -1) {
    switch (c) {
    case 'm':
      dev = optarg;
      break;
    ...
    case 'r':
        ...
        printf("receive: %s\n", my_msg);
        printf("receive: %c%c\n", hex[(my_msg[0] >> 4) & 0xf], hex[(my_msg[0] >> 0) & 0xf]);
